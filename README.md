...
| [ai-chat](/examples/ai-chat/)               | llm chat on `@yaebal/ai`: streamed draft replies, per-user memory, polite rate limits                                      | `pnpm --filter @yaebal/example-ai-chat dev`         |
...
| [ephemeral](/examples/ephemeral/)           | ephemeral menu commands, `ctx.replyEphemeral()` in groups, handle edits/deletes, private-chat fallback                     | `pnpm --filter @yaebal/example-ephemeral dev`       |
...
| [modular-router](/examples/modular-router/) | typed `define*()` file-based routes, a nested guard, `syncCommands`, `watchRoutes` hot-reload                              | `pnpm --filter @yaebal/example-modular-router dev`  |
...
| [audit-log](/examples/audit-log/)           | correlated, redacted-by-default logging, `applyRedaction`, `memorySink`, `chatSink`, `auditAdmin` ops commands             | `pnpm --filter @yaebal/example-audit-log dev`       |
...
| [@yaebal/ai](/packages/ai/)                         | llm superpowers: `ctx.ai.replyStream()` with native draft streaming, memory, limits, any provider — plus an mcp server and agent installer for ai-assisted development                                            |
...
| [@yaebal/cache](/packages/cache/)                   | `ctx.cache.get/set/wrap` — ttl memoization, stale-while-revalidate, sliding expiry, negative caching, prefix invalidation, typed key catalog                                                                      |
...
| [@yaebal/ephemeral](/packages/ephemeral/)           | `ctx.replyEphemeral()` — answer in a group so only the asker sees it: typed edit/delete handle over ephemeral messages, private-chat fallback, expiry policies                                                    |
| [@yaebal/filters](/packages/filters/)               | composable, type-narrowing update filters: `and`/`or`/`not`, deep links, async                                                                                                                                    |
| [@yaebal/guards](/packages/guards/)                 | reusable `bot.guard()` predicates: `isAdmin`, `hasPermission`, cached `membership()`, `guardOr`, anonymous-admin aware                                                                                            |
...
| [@yaebal/conversation](/packages/conversation/)     | await-style multi-step dialogs — coroutine by default, durable replay engine opt-in                                                                                                                               |
| [@yaebal/prompt](/packages/prompt/)                 | ask a question, handle the next message                                                                                                                                                                           |
| [@yaebal/router](/packages/router/)                 | typed file-based routing — `define*()` routes, load-time validation, nested guards, `watchRoutes` hot-reload                                                                                                      |
...
| [@yaebal/cron](/packages/cron/)                     | typed cron jobs: timezones, retries, catch-up, distributed locks, and a chat-native admin surface                                                                                                                 |
| [@yaebal/panel](/packages/panel/)                   | framework-agnostic operator panel with media, keyboards and event timeline                                                                                                                                        |
| [@yaebal/analytics](/packages/analytics/)           | typed `ctx.track(event, properties)` with auto-capture, privacy controls, an in-chat admin surface, and pluggable sinks: posthog, plausible, sqlite, clickhouse, http                                             |
| [@yaebal/audit-log](/packages/audit-log/)           | correlated, redacted-by-default audit logging — sinks, formatters, filters, sampling, `chatSink`, `auditAdmin`                                                                                                    |
