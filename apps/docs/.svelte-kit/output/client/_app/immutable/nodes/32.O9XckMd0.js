import{a as u,f}from"../chunks/CBlwNYzs.js";import"../chunks/Dwy-z0HI.js";import{s as e,f as y,e as b,$ as v,n as w}from"../chunks/CILFtpHi.js";import{h as S}from"../chunks/BElR-AbQ.js";import{C as t}from"../chunks/1IpLXSSe.js";var C=f(`<h1>@yaebal/scenes</h1> <p class="lead">step-by-step wizards over multiple messages. each step handles one incoming message; calling <code>ctx.scene.next()</code> advances to the next step on the following message. <code>ctx.scene.leave()</code> exits and restores normal handler routing.</p> <h2>install</h2> <!> <h2>usage</h2> <p>define your scenes as a <code>Record&lt;string, SceneDef&gt;</code>, pass them to <code>scenes()</code>, and install the plugin with <code>.install()</code>. enter a scene from
	...
	scene exits automatically without needing an explicit <code>leave()</code>. <br/><br/> <strong>state is in-memory by default.</strong> pass a persistent <code>StorageAdapter</code> to survive restarts — the same interface used by <code>@yaebal/session</code>.</div>`,1);function P(r){const i="pnpm add @yaebal/scenes",l=`import { Bot } from "@yaebal/core";
...
bot.start();`,h=`// a step that does NOT call ctx.scene.next() re-runs on the next message
...
};`,p=`// a step can switch to another scene entirely via ctx.scene.enter()
const defs: Record<string, SceneDef> = {
  a: {
    steps: [
      async (ctx: SceneContext) => {
...
};`,g=`// while in a scene all messages are consumed — /cancel won't fire its
...
async (ctx: SceneContext) => {
  ...
  // normal step logic...
  ctx.scene.next();
}`,m=`import { scenes } from "@yaebal/scenes";
...
}));`;var n=C();S("d6oasn",k=>{b(()=>{v.title="@yaebal/scenes — yaebal"})});var s=e(y(n),6);t(s,{code:i,title:"terminal",lang:"sh"});var o=e(s,6);t(o,{code:l,title:"bot.ts"});var c=e(o,18);t(c,{code:h,title:"validation.ts"});var a=e(c,6);t(a,{code:p,title:"switch-scene.ts"});var d=e(a,4);t(d,{code:m,title:"custom-storage.ts"});var x=e(d,4);t(x,{code:g,title:"cancel.ts"}),w(2),u(r,n)}export{P as component};
