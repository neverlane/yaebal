import{a as i,f as h}from"../chunks/CBlwNYzs.js";import"../chunks/Dwy-z0HI.js";import{s as e,f as p,e as b,$ as m,n as f}from"../chunks/CILFtpHi.js";import{h as u}from"../chunks/BElR-AbQ.js";import{C as t}from"../chunks/1IpLXSSe.js";var y=h(`<h1>runtime support</h1> <p class="lead">yaebal is fetch-first and has zero native-module dependencies in the core, so Bun, Deno, and
	...
	required.</p> <!> <h2>Bun</h2> <p>Pass the same fetch handler to <code>Bun.serve</code>. Bun's Node compatibility layer means
	you can also use <code>nodeWebhookCallback</code> with <code>Bun.serve</code>'s <code>node:http</code>-style API if you prefer, but the fetch path is simpler.</p> <!> <h2>Deno</h2> <p><code>Deno.serve</code> accepts a fetch handler directly. Import from npm via Deno's npm
	specifier support (<code>npm:@yaebal/core</code>) or use a local build.</p> <!> <h2>edge limitations</h2> <p>Edge runtimes (Cloudflare Workers, Deno Deploy, Vercel Edge) don't expose the filesystem or <code>worker_threads</code>, so anything that relies on those won't run there:</p> <ul><li><code>@yaebal/panel</code> — serves a static admin UI from disk; needs <code>fs</code></li> <li><code>@yaebal/router</code> — discovers handler files at startup; needs <code>fs</code></li> <li><code>@yaebal/workers</code> — spawns a <code>worker_threads</code> pool; not available at
		the edge</li> <li><code>nodeWebhookCallback</code> — uses <code>node:http</code> types; polyfills vary</li></ul> <p>Everything else — core, all filter/keyboard/session/i18n plugins, <code>webhook()</code> — runs fine at the edge.</p>`,1);function T(a){const s=`// Cloudflare Workers — fetch-first, runs at the edge
...
export default { fetch: webhook(bot) };`,c=`// Bun — built-in HTTP server
import { Bot } from "@yaebal/core";
import { webhook } from "@yaebal/web";
...
Bun.serve({ port: 8080, fetch: webhook(bot) });`,n=`// Deno — built-in HTTP server
import { Bot } from "@yaebal/core";
import { webhook } from "@yaebal/web";
...
Deno.serve({ port: 8080 }, webhook(bot));`;var o=y();u("jpkuyi",k=>{b(()=>{m.title="runtime support — yaebal"})});var d=e(p(o),14);t(d,{code:s,title:"worker.ts"});var r=e(d,6);t(r,{code:c,title:"server.ts"});var l=e(r,6);t(l,{code:n,title:"server.ts"}),f(8),i(a,o)}export{T as component};
