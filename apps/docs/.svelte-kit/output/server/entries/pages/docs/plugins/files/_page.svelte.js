...
bot.install(files());`;
  ...
  head("1gneyep", $$renderer, ($$renderer2) => {
    $$renderer2.title(($$renderer3) => {
      $$renderer3.push(`<title>@yaebal/files — yaebal</title>`);
    });
  });
  $$renderer.push(`<h1>@yaebal/files</h1> <p class="lead">resolve and download Telegram files. adds <code>ctx.files</code> with two helpers: get a CDN URL for any <code>file_id</code>, or buffer the whole file into a <code>Uint8Array</code>.</p> <h2>install</h2> `);
  Code($$renderer, { code: install, lang: "sh", title: "shell" });
  $$renderer.push(`<!----> <h2>registration</h2> <p>call <code>bot.install(files())</code> once. the plugin adds <code>ctx.files</code> to every subsequent handler in the chain.</p> `);
  Code($$renderer, { code: registration, title: "bot.ts" });
  ...
  Code($$renderer, { code: usage, title: "download-photo.ts" });
  $$renderer.push(`<!----> <div class="note"><strong>buffers the whole file.</strong> <code>download</code> uses <code>fetch</code> + <code>arrayBuffer()</code> — the entire file lands in memory before you get the bytes. for large files (video, documents) consider streaming from the URL returned by <code>fileLink</code> instead.</div>`);
}
export {
  _page as default
};
