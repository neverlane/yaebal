...
// → ["a".repeat(100), "a".repeat(100), "a".repeat(50)]`;
  head("wjbu3k", $$renderer, ($$renderer2) => {
    $$renderer2.title(($$renderer3) => {
      $$renderer3.push(`<title>split — yaebal</title>`);
    });
  });
  $$renderer.push(`<h1>split</h1> <p class="lead">break long messages into telegram-sized chunks. adds <code>ctx.sendLong</code> and <code>ctx.replyLong</code> to the context, and also exports a pure <code>split()</code> function
	for use outside a handler.</p> <h2>installation</h2> `);
  Code($$renderer, { code: install, lang: "sh", title: "terminal" });
  $$renderer.push(`<!----> <h2>basic usage</h2> <p>install the <code>splitter()</code> plugin with <code>bot.use()</code>. it adds <code>sendLong</code> and <code>replyLong</code> to every handler's context. both accept the same
	options as <code>ctx.send</code> / <code>ctx.reply</code>.</p> `);
  Code($$renderer, { code: basic, title: "bot.ts" });
  $$renderer.push(`<!----> <h2>custom chunk size</h2> <p>pass a number to <code>splitter()</code> to override the default 4096-character limit. useful for
	adding padding (captions, parse overhead) or during tests.</p> `);
  Code($$renderer, { code: customMax, title: "bot.ts" });
  $$renderer.push(`<!----> <h2>ctx.sendLong and ctx.replyLong</h2> <p>both return <code>Promise&lt;Message[]></code> — one <code>Message</code> per chunk sent.</p> `);
  Code($$renderer, { code: sendLong, title: "handler.ts" });
  $$renderer.push(`<!----> <h2>the pure split() function</h2> <p><code>split(text, max?)</code> is exported separately so you can use the splitting logic without
	a bot context — in tests, pre-processing pipelines, or anywhere you just need the chunks.</p> `);
  Code($$renderer, { code: pureFn, title: "util.ts" });
  $$renderer.push(`<!----> <h2>splitting rules</h2> <p>the algorithm prefers line breaks over hard cuts:</p> <ul><li>text shorter than or equal to <code>max</code> is returned as a single-element array</li> <li>lines are accumulated until the next line would exceed the limit, then a chunk is flushed</li> <li>a single line that is itself longer than <code>max</code> is hard-split at the byte boundary</li> <li>rejoining all chunks with <code>"\\n"</code> reconstructs the original text exactly</li></ul> `);
  Code($$renderer, { code: splitBehavior, title: "split-rules.ts" });
  $$renderer.push(`<!----> <h2>api</h2> <table><thead><tr><th>export</th><th>kind</th><th>description</th></tr></thead><tbody><tr><td><code>splitter(max?)</code></td><td>Plugin</td><td>installs <code>sendLong</code> / <code>replyLong</code> on the context</td></tr><tr><td><code>split(text, max?)</code></td><td>function</td><td>pure splitter — returns <code>string[]</code></td></tr><tr><td><code>MAX_MESSAGE_LENGTH</code></td><td>const</td><td><code>4096</code> — telegram's per-message text limit</td></tr><tr><td><code>SplitControl</code></td><td>interface</td><td>the shape added to the context by <code>splitter()</code></td></tr></tbody></table> <h2>SplitControl interface</h2> <table><thead><tr><th>method</th><th>returns</th><th>description</th></tr></thead><tbody><tr><td><code>sendLong(text, extra?)</code></td><td><code>Promise&lt;Message[]></code></td><td>send <code>text</code> split into chunks via <code>ctx.send</code></td></tr><tr><td><code>replyLong(text, extra?)</code></td><td><code>Promise&lt;Message[]></code></td><td>reply with <code>text</code> split into chunks via <code>ctx.reply</code></td></tr></tbody></table> <div class="note">chunks are sent sequentially, not in parallel — telegram preserves message order within a chat
	but parallel sends from one bot can arrive out of order.</div>`);
}
export {
  _page as default
};
