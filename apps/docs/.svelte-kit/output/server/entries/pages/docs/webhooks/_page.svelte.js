...
await bot.handleUpdate(update);`;
  head("31ylnl", $$renderer, ($$renderer2) => {
    $$renderer2.title(($$renderer3) => {
      $$renderer3.push(`<title>webhooks — yaebal</title>`);
    });
  });
  $$renderer.push(`<h1>webhooks</h1> <p class="lead">two webhook handlers — a fetch-style one and a node http one — both feeding the same
	...
	after a short delay if a poll fails. it resolves only when <code>bot.stop()</code> is called.</p> `);
  Code($$renderer, { code: polling, title: "polling.ts" });
  $$renderer.push(`<!----> <p>for production you usually want webhooks: Telegram POSTs each update to your URL, and you hand the
	...
	parses the JSON update, and dispatches it.</p> `);
  Code($$renderer, { code: fetchHandler, title: "webhook.ts" });
  $$renderer.push(`<!----> <h2>the secret check</h2> <p>when you set <code>secretToken</code>, the handler requires Telegram's <code>X-Telegram-Bot-Api-Secret-Token</code> header to match. the comparison is constant-time
	...
	check and same body cap, streamed off the incoming request.</p> `);
  Code($$renderer, { code: node, title: "server.ts" });
  $$renderer.push(`<!----> <h2>handleUpdate</h2> <p>both handlers ultimately call <code>bot.handleUpdate(update)</code>, which builds a <code>Context</code> and runs the middleware chain, sending any thrown error to your <code>onError</code> handler. you can also call it directly from a custom HTTP layer.</p> `);
  Code($$renderer, { code: handle, title: "handle.ts" });
  $$renderer.push(`<!----> <div class="note"><strong>register before the first update.</strong> the chain is realized and frozen on the first <code>handleUpdate</code> (or <code>start</code>), so attach all middleware and plugins before
	then.</div> <h2>deploy: Cloudflare Workers</h2> `);
  Code($$renderer, { code: workers, title: "worker.ts" });
  $$renderer.push(`<!----> <h2>deploy: Bun</h2> `);
  Code($$renderer, { code: bun, title: "server.ts" });
  $$renderer.push(`<!---->`);
}
export {
  _page as default
