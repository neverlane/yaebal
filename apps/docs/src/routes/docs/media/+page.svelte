...
<h1>media</h1>
<p class="lead">
	one uniform way to point at a file — local path, url, in-memory buffer, or an existing Telegram
	file_id — and the Api layer picks the right wire form.
</p>
...
<p>
	<code>encodeRequest</code> decides the encoding per request. if no <code>path</code> or
	<code>buffer</code> is present, the body is JSON and any <code>url</code>/<code>fileId</code> media
	is inlined to its string:
</p>
<Code code={json} title="api.ts" />
<p>
	the moment a <code>path</code> or <code>buffer</code> appears anywhere in the top-level params, the
	whole request switches to <code>multipart/form-data</code>. each upload is written to a generated
	field and the param points at it with <code>attach://</code>:
</p>
<Code code={multipart} title="api.ts" />

<div class="note">
	<strong>top-level only.</strong> media is handled at the top level of the params. nested
	<code>MediaSource</code> (e.g. inside <code>sendMediaGroup</code>'s <code>media[]</code>) is
