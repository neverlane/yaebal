...
<table>
	<thead>
		<tr><th>export</th><th>kind</th><th>signature</th><th>description</th></tr>
	</thead>
	<tbody>
		<tr>
			...
			<td>returns the plugin. install with <code>bot.install(files())</code>.</td>
		</tr>
		<tr>
			<td><code>FilesControl</code></td>
			<td>interface</td>
			<td>—</td>
			<td>the type of <code>ctx.files</code>.</td>
		</tr>
	</tbody>
</table>
...
<table>
	...
	<tbody>
		<tr>
			<td><code>fileLink</code></td>
			<td><code>(fileId: string) =&gt; Promise&lt;string&gt;</code></td>
			<td>calls <code>getFile</code> then returns the full CDN URL. throws if <code>file_path</code> is absent.</td>
		</tr>
		<tr>
			<td><code>download</code></td>
			<td><code>(fileId: string) =&gt; Promise&lt;Uint8Array&gt;</code></td>
			<td>resolves the URL via <code>fileLink</code> then <code>fetch</code>es it into memory. throws on a non-2xx response.</td>
		</tr>
	</tbody>
</table>
...
<div class="note">
	<strong>buffers the whole file.</strong> <code>download</code> uses <code>fetch</code> + <code>arrayBuffer()</code> — the entire file lands in memory before you get the bytes. for large files (video, documents) consider streaming from the URL returned by <code>fileLink</code> instead.
</div>
