...
<h1>@yaebal/panel</h1>
<p class="lead">
	an operator panel: view incoming private-chat messages and reply from the browser. ships as a
	self-contained fetch handler — mount it on any http framework. the
	<code>recorder()</code> plugin feeds the store; <code>panelHandler()</code> serves the UI and a
	small REST API.
</p>
...
<h2>mounting on hono (or any fetch-based framework)</h2>
<Code code={hono} title="hono.ts" />

<h2>authentication</h2>
<p>
	<code>panelHandler</code> requires a non-empty <code>token</code>. it is checked on every
...
<Code code={bearerAuth} lang="text" title="auth" />

<h2>panel API routes</h2>
<Code code={apiRoutes} lang="text" title="routes" />

<h2>persistent storage</h2>
<p>
	<code>MemoryPanelStore</code> is the default — it keeps up to 1000 messages per chat in memory
	and is lost on restart. for production, implement <code>PanelStore</code> against your database.
</p>
<Code code={customStore} title="store.ts" />

<h2>what the recorder captures</h2>
<p>
	<code>recorder(store)</code> is a standard yaebal plugin. it records incoming <em>private-chat
...
<table>
	...
	<tbody>
		<tr><td><code>panelHandler(api, store, options)</code></td><td>function</td><td>returns a <code>(Request) =&gt; Promise&lt;Response&gt;</code> handler</td></tr>
		<tr><td><code>recorder(store)</code></td><td>Plugin</td><td>logs incoming private text into the store</td></tr>
		<tr><td><code>MemoryPanelStore</code></td><td>class</td><td>in-memory <code>PanelStore</code>; implements <code>record</code>, <code>chats</code>, <code>history</code></td></tr>
		<tr><td><code>PanelStore</code></td><td>interface</td><td>implement to persist conversations</td></tr>
		<tr><td><code>PanelMessage</code></td><td>interface</td><td><code>{"{ direction: 'in' | 'out'; text: string; date: number }"}</code></td></tr>
		<tr><td><code>PanelChat</code></td><td>interface</td><td><code>{"{ id: number; name: string; lastText: string; lastDate: number }"}</code></td></tr>
		<tr><td><code>PanelOptions</code></td><td>interface</td><td><code>{"{ token: string }"}</code> — required auth token</td></tr>
		<tr><td><code>PANEL_HTML</code></td><td>string</td><td>the raw HTML of the panel UI (exported for custom serving)</td></tr>
	</tbody>
</table>

<div class="note">
	the panel HTML is a single self-contained page — no external assets, no CDN. it polls every 4
	seconds for new chats and refreshes the open conversation. for a production deployment, put the
	panel behind a reverse proxy with TLS and restrict the <code>token</code> to a long random value.
</div>
