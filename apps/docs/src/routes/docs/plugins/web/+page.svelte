...
<p class="lead">
	...
	webhooks. no long-polling: each incoming <code>Request</code> becomes one update.
</p>

<h2>install</h2>
<Code code={install} title="terminal" lang="sh" />

<h2>edge: a fetch handler</h2>
<p>
	<code>webhook(bot)</code> returns a standard <code>(Request) =&gt; Promise&lt;Response&gt;</code>.
	Drop it into any runtime that speaks fetch — that's all an edge bot needs.
</p>
<Code code={workers} title="worker.ts" />

<h2>standalone: serve on Bun / Deno</h2>
<p>
	For a long-running web server (not edge), <code>serve()</code> uses the runtime's native fetch
	server. On Node, use <code>nodeWebhookCallback</code> from <code>@yaebal/core</code> instead.
</p>
...
<p>
	...
	you pass to <code>webhook()</code> — it's checked in constant time on every request.
</p>
...
<table>
	...
	<tbody>
		<tr><td><code>webhook(bot, options?)</code></td><td>fetch handler for edge/web runtimes</td></tr>
		<tr><td><code>serve(bot, options?)</code></td><td>standalone server on Bun or Deno</td></tr>
		<tr><td><code>setWebhook(bot, url, options?)</code></td><td>register the webhook with Telegram</td></tr>
		<tr><td><code>deleteWebhook(bot, dropPending?)</code></td><td>remove the webhook</td></tr>
	</tbody>
</table>

<div class="note">
	The whole package is <code>fetch</code>-first with zero <code>node:</code> imports, so it (and the
	core it wraps) runs on Node, Bun, Deno and edge. Looking for the operator dashboard? That moved to
