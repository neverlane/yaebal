...
| **`@yaebal/ai`** ✅ | llm runtime + ai dev tooling. runtime: `ctx.ai.replyStream()` streams answers via `sendMessageDraft` in private chats (native "Thinking…") and throttled edits elsewhere, `@yaebal/split` past 4096, per-user-per-chat memory on `sklad` adapters, `"20/h"` limits (`AiLimitError`), provider-agnostic `AiModel` (openai-compatible + anthropic fetch adapters, lazy vercel ai sdk bridge, custom streams). tooling: stdio mcp server (exact bot api schema from `@yaebal/types/schema.json`, plugin catalog, docs search, examples), `npx @yaebal/ai` installer for 9 agent targets, claude code plugin generated into `packages/ai/claude-plugin/` | `sklad`, `split`, `types` | `derive` → `ctx.ai` | puregram stream (streaming only; no memory/tools/groups) |
...
| **`@yaebal/inline-results`** ✅ | typed builders for every `InlineQueryResult` variant (+ `cached.*` for file_id-backed ones) and `InputMessageContent` — required fields positional, rest a typed options object; `caption`/`message_text` accept `FormattedText` decomposed by `bot.api` | `types` | export | @gramio InlineQueryResult builders |
...
| **`@yaebal/router`** ✅ | file-based routing: typed `define*()` route files (`commands/`, `on/`, `hears/`, `use/`), load-time validation (update-type/command-name checks with did-you-mean, duplicate detection), nested `_guard.ts` directories, an optional `@yaebal/commands` menu bridge, `watchRoutes()` dev hot-reload | `commands` (optional) | @gramio/autoload + storona |
