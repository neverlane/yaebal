...
| [modular-router](./modular-router/) | `@yaebal/example-modular-router` | typed `define*()` file-based routes (`commands`/`on`/`hears`/`use`), a nested `_guard.ts`, `syncCommands`, `watchRoutes` hot-reload | `pnpm --filter @yaebal/example-modular-router dev` |
