...
const bot = new Bot(token)
	...
	.install(
		mediaGroup(async (ctx, messages) => {
			...
			await ctx.reply(`album captured: ${messages.length} item(s), ${photoCount} photo(s), ${videoCount} video(s).`);
		}),
	);
...
bot.command("start", (ctx) =>
	...
	.command("cache", async (ctx) => {
		await cache.photo(ctx, "poster:v1", media.url("https://picsum.photos/seed/yaebal-cache/900/600"), {
	...
	.command("preview", (ctx) => {
		...
		return ctx.sendDocument(media.buffer(new TextEncoder().encode(svg), "media-studio-preview.svg"), {
	...
	.command("long", (ctx) => {
		const report = Array.from(
			{ length: 80 },
			(_, index) => `line ${index + 1}: media job shard ${index % 7} ok, cache warm, album queue clear.`,
		).join("\n");
